...
if ($myId >= 1) {
	...
	$editJ = (filter_input ( INPUT_GET, 'editJ', FILTER_SANITIZE_NUMBER_INT )) ? filter_input ( INPUT_GET, 'editJ', FILTER_SANITIZE_NUMBER_INT ) : "new";
	$table = $myId . "Journal";
	...
	if (filter_input ( INPUT_POST, 'delJFinal', FILTER_SANITIZE_NUMBER_INT ) == 1) {
		$delJF = filter_input ( INPUT_POST, 'delJFinal', FILTER_SANITIZE_NUMBER_INT );
	...
	if (filter_input ( INPUT_POST, 'iUp', FILTER_SANITIZE_NUMBER_INT )) {
		...
		$interpretation = htmlentities ( filter_input ( INPUT_POST, 'interpretation', FILTER_SANITIZE_STRING ), ENT_QUOTES );
		$putI = $db->prepare ( "UPDATE $table SET interpretation = ? WHERE id = ?" );
		$putI->execute ( array (
				$interpretation,
				$iUp
		) );
	...
	if (filter_input ( INPUT_POST, 'dUp', FILTER_SANITIZE_STRING )) {
		...
		$dream = htmlentities ( filter_input ( INPUT_POST, 'dream', FILTER_SANITIZE_STRING ), ENT_QUOTES );
		...
		if ($dream != "" && $dream != " ") {
			if ($dUp == "new") {
				$put = $db->prepare ( "INSERT INTO $table VALUES" . "(NULL, ?, ?, ?, '', 'x.jpg', 'x.jpg', '0', '0')" );
				$put->execute ( array (
						$date,
						$title,
						$dream
				) );
				$getId = $db->prepare ( "SELECT id FROM $table WHERE date = ? AND title = ? AND dream = ? ORDER BY id DESC LIMIT 1" );
				$getId->execute ( array (
						$date,
						$title,
						$dream
				...
				$put = $db->prepare ( "UPDATE $table SET date = ?, title = ?, dream = ? WHERE id = ?" );
				$put->execute ( array (
						$date,
						$title,
						$dream,
			...
			if (! empty ( $_FILES ["pic1"] ["tmp_name"] )) {
				$image = $_FILES ["pic1"] ["tmp_name"];
				list ( $width, $height ) = (getimagesize ( $image ) != null) ? getimagesize ( $image ) : null;
				if ($width != null && $height != null) {
					$imageType = getPicType ( $_FILES ["pic1"] ['type'] );
					$imageName = $time . "." . $imageType;
					processPic ( $myId, $imageName, $image );
					processThumbPic ( $myId, $imageName, $image );
					$p1stmt = $db->prepare ( "UPDATE $table SET pic1 = ? WHERE id = ?" );
			...
			if (! empty ( $_FILES ["pic2"] ["tmp_name"] )) {
				...
				if ($width != null && $height != null) {
					...
					processPic ( $myId, $imageName, $image );
					processThumbPic ( $myId, $imageName, $image );
					$p2stmt = $db->prepare ( "UPDATE $table SET pic2 = ? WHERE id = ?" );
	...
	if ($whichJ == "new") {
		...
		if ($editJ != 'new') {
			...
			$interpretation = html_entity_decode ( $getR ['interpretation'], ENT_QUOTES );
			$pic1 = $getR ['pic1'];
			$pic2 = $getR ['pic2'];
		} else {
		...
		$year = (filter_input ( INPUT_GET, 'year', FILTER_SANITIZE_NUMBER_INT )) ? filter_input ( INPUT_GET, 'year', FILTER_SANITIZE_NUMBER_INT ) : date ( "Y", $time );
		$month = (filter_input ( INPUT_GET, 'month', FILTER_SANITIZE_NUMBER_INT )) ? filter_input ( INPUT_GET, 'month', FILTER_SANITIZE_NUMBER_INT ) : date ( "m", $time );
		...
		$getDates1 = $db->prepare ( "SELECT date FROM $table ORDER BY DATE LIMIT 1" );
		$getDates1->execute ();
		$getDates1R = $getDates1->fetch ();
		$YStart = date ( "Y", $getDates1R ['date'] );
		$getDates2 = $db->prepare ( "SELECT date FROM $table ORDER BY date DESC LIMIT 1" );
		$getDates2->execute ();
		...
		for($y = $YStart; $y <= $YEnd; ++ $y) {
			...
			$getDates3 = $db->prepare ( "SELECT COUNT(*) FROM $table WHERE date >= ? AND date <= ?" );
		...
		if ($yCount >= 1) {
			...
			foreach ( $years as $y ) {
				...
				for($m = 1; $m <= 12; ++ $m) {
					...
					$getDates4 = $db->prepare ( "SELECT COUNT(*) FROM $table WHERE date >= ? AND date < ?" );
				...
				foreach ( $months as $m ) {
					echo "<a href='index.php?page=journal&viewJ=interpret&year=$y&month=$m' style='color:$linkColor; font-weight:bold; font-size:1.25em; padding:0px 10px;'>" . $MONTHS [$m] . "</a>\n";
				}
				echo "</div>\n";
			}
		}
		...
		$symbolArray = array ();
		$stmt = $db->prepare ( "SELECT id, symbol FROM symbolDictionary ORDER BY length DESC" );
		$stmt->execute ();
		...
		$show = $db->prepare ( "SELECT * FROM $table WHERE date >= ? AND date < ? ORDER BY date" );
		$show->execute ( array (
				$showS,
				$showE
		) );
		while ( $showR = $show->fetch () ) {
			...
			$interpretation = html_entity_decode ( trim ( $showR ['interpretation'] ), ENT_QUOTES );
			$pic1 = $showR ['pic1'];
			$pic2 = $showR ['pic2'];
			...
			foreach ( $dreamList as $kd => &$vd ) {
				...
				if (! in_array ( strtolower ( $vdTrimmed ), $SKIPWORDS )) {
					foreach ( $symbolArray as $vs ) {
						if (str_contains ( $vd, $vs [1] ) || str_contains ( $vd, strtolower ( $vs [1] ) )) {
			...
			while ( preg_match ( '/\[([1-9][0-9]*)\]/', $dream, $match ) ) {
				$symId = $match [1];
				$geti4 = $db->prepare ( "SELECT symbol, definition, source FROM symbolDictionary WHERE id = ?" );
				$geti4->execute ( array (
						$symId
				) );
				$gi4 = $geti4->fetch ();
				...
				$replace = "<span style='color:$linkColor; cursor:pointer;' class='flip'>" . strtoupper ( $gSymbol ) . "</span>";
			...
			$displayDream = str_replace ( "\n", "<br><br>", make_links_clickable ( $dream, $linkColor ) );
		...
		if ($yCount >= 1) {
			...
			foreach ( $years as $y ) {
				...
				echo "<div id='showMonth$y' style='display:none; width:100%; text-align:center;'>\n";
				foreach ( $months as $m ) {
					echo "<a href='index.php?page=journal&viewJ=review&year=$y&month=$m' style='color:$linkColor; font-weight:bold; font-size:1.25em; padding:0px 10px;'>" . $MONTHS [$m] . "</a>\n";
				}
				echo "</div>\n";
			}
		}
		...
		while ( $showR = $show->fetch () ) {
			...
			$date = date ( "Y-m-d", $showR ['date'] );
			$title = html_entity_decode ( trim ( $showR ['title'] ), ENT_QUOTES );
			$dream = make_links_clickable ( html_entity_decode ( trim ( $showR ['dream'] ), ENT_QUOTES ), $linkColor );
			$interpretation = html_entity_decode ( trim ( $showR ['interpretation'] ), ENT_QUOTES );
			$pic1 = $showR ['pic1'];
			$pic2 = $showR ['pic2'];
			...
			echo "<div style='margin-bottom: 20px; font-size:.75em;'><a href='index.php?page=journal&viewJ=new&editJ=$id'>Edit Dream</a></div>\n";
			echo "<div style='margin-bottom: 10px; font-weight:bold; font-size:1.25em; color:$highlightColor;'>$title</div>\n";
			echo "<div style='margin-bottom: 10px;'><span style='font-weight:bold;'>Dream:</span><br>" . nl2br ( $dream ) . "</div>\n";
			echo "<div style='margin-bottom: 10px;'><span style='font-weight:bold;'>Interpretation:</span><br>" . nl2br ( $interpretation ) . "</div>\n";
			if (file_exists ( "cmPics/$myId/$pic1" )) {
				echo "<div style='margin-bottom: 10px;'><image src='cmPics/$myId/$pic1' alt='' style='border:1px solid $highlightColor; padding:5px;' /></div>\n";
			}
			if (file_exists ( "cmPics/$myId/$pic2" )) {
